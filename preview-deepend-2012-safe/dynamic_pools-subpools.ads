...
with Sys.Storage_Pools.Subpools; use Sys.Storage_Pools.Subpools;
...
package Dynamic_Pools.Subpools is
   ...
   type Dynamic_Unbounded_Subpool_Handle
     (Mode : Mode_Kinds;
      Declaring_Task_Is_Owner : Boolean) is
     new Dynamic_Subpool_Handle with private;

--   type Dynamic_Subpool_Handle is access all Dynamic_Unbounded_Subpool;
   ...
   overriding function Pool_Needs_Finalization
   ...
   overriding function Subpool_Needs_Finalization
     (Subpool : Dynamic_Unbounded_Subpool_Handle) return Boolean;
      --  Returns True if Unchecked_Deallocate_Storage has not been called for
      --  the subpool, and neither Unchecked_Deallocate_Objects or
      --  Unchecked_Deallocate_Storage has been called for any of the pools
      --  ancestors. Returns false otherwise.
   ...
   function Is_Owner
     (Subpool : Dynamic_Unbounded_Subpool_Handle;
      T : Task_Id := Current_Task) return Boolean;
   pragma Precondition (Subpool_Needs_Finalization (Subpool));
   --  Returns True if the specified task owns the pool/subpool and thus is
   --  allowed to allocate from it.
   ...
   pragma Precondition
     (Is_Owner
        (Dynamic_Unbounded_Subpool_Handle (Subpool),
         Current_Task) and
        Subpool_Needs_Finalization
        (Dynamic_Unbounded_Subpool_Handle (Subpool)));
   ...
   procedure Unchecked_Deallocate_Objects
     (Subpool : in out Dynamic_Unbounded_Subpool_Handle);
   pragma Precondition (Subpool_Needs_Finalization (Subpool));
   --  This call first has the effect of calling Unchecked_Deallocate_Storage
   --  for all subpools of the pool. It then has the effect of calling
   --  Dynamic_Pools.Unchecked_Deallocate_Objects (the parent type
   ...
   pragma Precondition (Subpool_Needs_Finalization (Subpool));
   pragma Postcondition
     (not Objects_Need_Finalization (Subpool) --  and
...
private
   ...
   type Dynamic_Unbounded_Subpool;
   type Dynamic_Subpool_Access is access Dynamic_Unbounded_Subpool;
   type Access_To_Subpool_Access is access all Subpool_Access;

   package Subpool_Vector is new
     Ada.Containers.Vectors (Index_Type => Positive,
                             Element_Type => Subpool_Access);

   type Dynamic_Unbounded_Subpool is new
   ...
   type Dynamic_Pool_With_Subpools
     ...
     new Dynamic_Pool with
      record
         Default_Subpool : aliased Subpool_Access;
      end record;
   ...
   type Dynamic_Unbounded_Subpool_Handle
     (Mode : Mode_Kinds;
      Declaring_Task_Is_Owner : Boolean) is new
     Dynamic_Subpool_Handle with null record;
