...
package Dynamic_Pools.Subpools is
   ...
   type Dynamic_Unbounded_Subpool
     (Mode : Mode_Kinds;
      Declaring_Task_Is_Owner : Boolean) is new Dynamic_Subpool with private;
   ...
   pragma Precondition (Is_Owner (Subpool, Current_Task) and
                        Subpool_Needs_Finalization (Subpool));
   ...
   --  BJM Can't implement easily. No easy way to get Pool from a
   --  Subpool object. You need a Subpool_Handle to do that.
   --  A convenience function should be added to Subpools package
...
private
   ...
   type Dynamic_Unbounded_Subpool
     (Mode : Mode_Kinds;
      Declaring_Task_Is_Owner : Boolean) is new Dynamic_Subpool with
      record
         Storage : aliased Pool_Storage_Access;
      end record;
